...
export function generateImports({parseResult, config, typePaths}) {
    return parseResult.newParts.map(part => {
        ...
        if (parseResult.type === 'SignalFile') {
            if (config.addons.indexOf(part.identifier) > -1) {
        ...
        if (shouldCreateFile) {
            ...
            if (config.indentation !== '2 spaces') {
                if (config.indentation === '4 spaces') {
