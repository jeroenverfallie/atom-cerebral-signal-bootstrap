...
function parse(content) {
    ...
    const replaceOutputs = new RegExp('(\\W)(' + outputs.join('|') + ')\\:', 'g');
    const preparedSignal = outputs.length ? signal.replace(replaceOutputs, '$1"$2":') : signal;
...
function createDefinedSignal(content, context) {
    ...
    for (var key in context) {
        output = output.replace(new RegExp('(\\W)' + key + '(\\W)', 'g'), '$1' + JSON.stringify(context[key]) + '$2');
    }
